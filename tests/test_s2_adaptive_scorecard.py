...
def test_implementation_repair_mode_can_reuse_failed_integration_point() -> None:
    ...
    context = {
        "direction_id": "direction_x",
        "recent_failures": [{"mechanism_axis": "routing", "integration_point": "projector", "failure_class": "proxy_negative"}],
        "attempt_counters": {"same_direction_proxy_failures": 2, "max_same_direction_proxy_failures": 2},
        "dragging_datasets": [],
    }
    policy = build_s2_adaptive_policy(context, _config())
    scorecard = build_s2_variant_scorecard(
        ...
        selected_variant=variant,
        evidence_quality={"support_coverage": {"paper": 2, "code": 2}},
        variant_fingerprint={"variant_fingerprint": "fp_old_point", "is_repeat": True, "mode": "implementation_repair"},
        planner_memory={"entries": []},
        feedback=[],
        feedback_context=context,
        adaptive_policy=policy,
    ...
    assert gate["gate"] == "pass"
    assert gate["checks"]["force_new_integration_point_satisfied"] is True
