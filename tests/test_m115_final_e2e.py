    ...
    *,
    ...
    def run_step(*, name, command, working_dir, retry_policy=None, **kwargs):
        ...
        if name == "proxy_baseline_train":
            ...
            target = run_repo / f"local/auto_research_runs/{run_id}/proxy/results/mmlu-redux"; payload=json.dumps({"model":"Rosetta","dataset":"mmlu-redux","answer_method":"generate","overall_accuracy":proxy_accuracy}); subprocess.run([sys.executable,"-c",script,str(target),"Rosetta_mmlu-redux_generate_summary.json",payload],check=True); (target / "prediction_outputs.jsonl").write_text("\n".join([json.dumps({"prediction":"Answer: A","answer":"A"}), json.dumps({"prediction":"Answer: B","answer":"B"})]) + "\n", encoding="utf-8")
        elif name == "activation_smoke_eval_mmlu-redux":
            target = run_repo / f"local/auto_research_runs/{run_id}/proxy/activation_smoke_disabled/results/mmlu-redux"; payload=json.dumps({"model":"Rosetta","dataset":"mmlu-redux","answer_method":"generate","overall_accuracy":max(0.0, proxy_accuracy - 0.02)}); subprocess.run([sys.executable,"-c",script,str(target),"Rosetta_mmlu-redux_generate_summary.json",payload],check=True); (target / "prediction_outputs.jsonl").write_text("\n".join([json.dumps({"prediction":"Answer: B","answer":"A"}), json.dumps({"prediction":"Answer: A","answer":"B"})]) + "\n", encoding="utf-8")
        elif name == "train":
