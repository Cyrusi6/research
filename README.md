...
- `plan/attempts/<attempt_id>/trial_spec/<trial_spec_hash>.json`: the immutable `TrialSpec v8` projection frozen by the reservation; `plan/trial_spec.json` is diagnostic only.
- `meta/attempts/<attempt_id>.json`: `AttemptRecord v9`, a rebuildable projection reused across crash/resume, resource resume, and implementation repair.
...
- `meta/research_events.sqlite3`: the sole authoritative Event v9 SQLite-WAL store; `ResearchState v9`, attempts, results, routes, phase outcomes, command lifecycle, and aggregates are rebuildable projections.
...
`TrialSpec v8` freezes a scientific `ProxyDecisionPolicy v2`, `ReadinessCheckPlan v1`, `EvidenceDerivationPlan v1`, and exact `PhaseCommandPlan v3` before execution. The command plan fixes typed `argv[]`, `cwd`, environment overrides, inherited-environment allowlists, dependencies, conditions, physical raw outputs, and retry/resource policy. The derivation plan fixes decoder ID/version/semantic and implementation hashes, immutable decoder bytes, ordered physical source bindings, coverage/canonicalization, and the exact normalized output set. `ExperimentRunner` executes the frozen invocation with `subprocess.Popen(..., shell=False, env=...)`; display strings never participate in execution. Producer reports cannot change thresholds, readiness predicates, evidence lineage, decisions, or routes.
...
Every command must match the frozen `PhaseCommandPlan v3` DAG and its typed invocation before `PhaseCommandStarted`. `PhaseCommand v4` and `PhaseRunReceipt v5` bind the command, authorization, stdout/stderr, and each physical raw output to the current Attempt/generation/phase. The only scientific derivation chain is `EvidenceDerivationPlan v1` → completed physical receipts/raw outputs → constrained Core derivation → `EvidenceDerivationManifest v2` → the derive receipt's structured `derivation_ref/hash` → the same reference in `EvidenceManifest v6` → `ProxyOutcome v4` or `TrialResult v6`. Direct/self derivation from normalized outputs is removed.

All precommit, transaction, reducer/rebuild, query, and Gate checks use the same read-only immutable-derivation validator. It rereads frozen decoder bytes and physical receipt outputs, deterministically recomputes normalized bytes in memory, and byte-compares the derivation manifest, derive receipt, EvidenceManifest, and evidence blobs. Validation never writes CAS/SQLite, reads current source files as decoder authority, repairs missing blobs, or reruns commands; missing or damaged raw, decoder, derivation, receipt, or normalized bytes fail closed.

Activation and readiness are independently receipt-derived. Command exit zero proves only command completion. `ActivationEvidence v4` requires actual enabled/disabled measurements, observed surface coverage, and the frozen delta threshold; expected surfaces cannot be copied into observed surfaces. `FullS3Readiness v4` is computed from the frozen `ReadinessCheckPlan v1` and its exact raw receipt bindings. Valid semantic `BLOCKED` activation/readiness routes deterministically to `REPAIR_IMPLEMENTATION`, starts no full command, preserves the reservation, and consumes no method outcome; integrity drift routes `BLOCK_INTEGRITY`, while only a committed `RUN_FULL` authorizes full execution.

Planner pools, scorecards, patch diagnostics, producer proxy-policy reports, mutable result summaries, and human-readable projections remain diagnostic artifacts only. Runtime stages do not infer identity, observations, budgets, authorization, or routing from those files. Projects without DirectionSpec v3, VariantSpec v4, TrialSpec v8, and Event/Attempt/ResearchState v9 authority must restart from S1; no dual-read or migration exists. See `docs/authoritative_s1_s3_contracts.md` and `docs/regression_test_migration.md`.
...
M1.1.5.3 replaces the M1.1.5.2 direct/self derivation authority with the single immutable chain above and makes readiness a frozen, receipt-derived semantic decision. M1.1.5.2's `705 passed, 2 skipped` figures are a historical pre-v9 checkpoint, not M1.1.5.3 acceptance. The final v9/v8/v6/v5 suite reports `747 passed, 2 skipped, 0 failed` in all four required environments: normal, proxy-cleared, empty-HOME/empty-HF-cache/PATH-without-Codex, and CPU-only/PATH-without-`nvidia-smi`. The two skips remain the existing optional torch/transformers checks.
