...
- `plan/attempts/<attempt_id>/trial_spec/<trial_spec_hash>.json`: the immutable `TrialSpec v9` projection frozen by the reservation; `plan/trial_spec.json` is diagnostic only.
- `meta/attempts/<attempt_id>.json`: `AttemptRecord v10`, a rebuildable projection reused across crash/resume, resource resume, and implementation repair.
- `experiment/results/trial_result.json`: `TrialResult v6`, the latest committed result deterministically decoded from immutable row-level evidence.
- `meta/route_outcome.json`: `RouteOutcome v4`, the latest deterministic route projection.
- `meta/research_events.sqlite3`: the sole authoritative Event v10 SQLite-WAL store; `ResearchState v10`, attempts, results, routes, phase outcomes, command lifecycle, and aggregates are rebuildable projections.
...
`TrialSpec v9` freezes a scientific `ProxyDecisionPolicy v2`, `ReadinessCheckPlan v2`, `EvidenceDerivationPlan v2`, and exact `PhaseCommandPlan v4` before execution. Its `DecoderDescriptor v2` points to a content-addressed `DecoderImplementationBundle v1` containing the actual declarative `DecoderProgram v1`, runtime ABI, entrypoint, dependency contract, canonicalization, measurement mapping, pairing, activation, readiness, and output-identity semantics. The implementation hash is the executable-semantics artifact hash; replay never obtains historical decoder behavior from the current runtime registry or source file. `ExperimentRunner` executes frozen typed `argv[]`, `cwd`, environment overrides, inherited-environment allowlists, dependencies, conditions, physical raw outputs, and retry/resource policy with `subprocess.Popen(..., shell=False, env=...)`.
...
Every command must match the frozen `PhaseCommandPlan v4` DAG and its typed invocation before `PhaseCommandStarted`. `PhaseCommand v5` and `PhaseRunReceipt v5` bind the command, authorization, stdout/stderr, and each physical raw output to the current Attempt/generation/phase. The only scientific derivation chain is `EvidenceDerivationPlan v2` → completed physical receipts/raw outputs → the journal-owned frozen decoder execution → `EvidenceDerivationManifest v3` → the derive receipt's structured `derivation_ref/hash` → the same reference in `EvidenceManifest v6` → `ProxyOutcome v4` or `TrialResult v6`. Direct/self derivation and pre-journal decoder execution are removed.

All precommit, `PhaseCommandCompleted`, transaction, reducer/rebuild, query, and Gate checks use the same read-only immutable-derivation validator. It executes the frozen declarative decoder artifact over the ordered exact physical source set, deterministically recomputes normalized bytes in memory, and byte-compares the derivation manifest, derive receipt, EvidenceManifest, and evidence blobs. The producing decoder runs only inside the command journal runner; durable-receipt recovery validates and commits only the missing Completed event. Validation never writes CAS/SQLite, reads current source files or registries as historical decoder authority, repairs missing blobs, or reruns commands.

Activation and readiness are independently receipt-derived. Command exit zero proves only command completion. `ActivationEvidence v4` requires explicit enabled/disabled measurements and observed-surface facts; expected surfaces cannot be copied into observed surfaces. `FullS3Readiness v4` is computed from `ReadinessCheckPlan v2` using the exact ordered inventory of frozen authority roles and check IDs. Missing, extra, duplicate, reordered, or cross-authority inputs fail closed. Valid semantic `BLOCKED` activation/readiness routes deterministically to `REPAIR_IMPLEMENTATION`; integrity drift commits a replayable SQLite `BLOCK_INTEGRITY`; only a committed `RUN_FULL` authorizes full execution.

Planner pools, scorecards, patch diagnostics, producer proxy-policy reports, mutable result summaries, and human-readable projections remain diagnostic artifacts only. Runtime stages do not infer identity, observations, budgets, authorization, or routing from those files. Projects without DirectionSpec v3, VariantSpec v4, TrialSpec v9, and Event/Attempt/ResearchState v10 authority must restart from S1; no dual-read or migration exists. See `docs/authoritative_s1_s3_contracts.md` and `docs/regression_test_migration.md`.
...
M1.1.5.3 established the single physical-receipt derivation/readiness chain at commit `1fd4e84`, but that commit was a migration checkpoint rather than final acceptance. M1.1.5.3.1 freezes the actual declarative decoder execution semantics, moves producing derivation behind journal idempotency, validates semantic raw-to-normalized equality at `PhaseCommandCompleted`, enforces exact ordered readiness/activation authority sets, and makes pre-derive integrity failure a replayable SQLite route. Earlier `747 passed, 2 skipped` figures certify only the pre-v10 contracts and are not reused as current evidence.

The M1.1.5.3.1 v10/v9 suite collects 781 tests and reports `779 passed, 2 skipped, 0 failed` in each required local environment: normal, proxy-cleared, empty HOME/HF cache with PATH excluding Codex, and CPU-only with PATH excluding `nvidia-smi`. The two skips remain the pre-existing optional torch/transformers forward-probe checks; no skip or xfail was added.
