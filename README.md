...
- `plan/attempts/<attempt_id>/trial_spec/<trial_spec_hash>.json`: the immutable `TrialSpec v5` projection frozen by the reservation; `plan/trial_spec.json` is diagnostic only.
- `meta/attempts/<attempt_id>.json`: `AttemptRecord v6`, a rebuildable projection reused across crash/resume, resource resume, and implementation repair.
...
- `meta/research_events.sqlite3`: the sole authoritative Event v6 SQLite-WAL store; snapshots, attempts, results, routes, phase outcomes, command lifecycle, and aggregates are rebuildable projections.
...
`TrialSpec v5` freezes a scientific `ProxyDecisionPolicy v1` before reservation. The policy contains metrics, objective, paired aggregation, exact dataset/seed/role coverage, thresholds, activation/readiness requirements, evidence kinds, and deterministic route semantics—but no Attempt, generation, implementation, producer, or phase identity. `ProxyPhaseStarted` derives a separate `ProxyEvaluationBinding v1` that binds the frozen policy to the current Attempt generation, implementation/input hashes, phase execution, command plan, sample/evaluator contracts, and provenance mode. The reducer-owned classifier consumes only this policy, binding, and immutable raw evidence; producer policy reports are diagnostic and cannot change thresholds or routes.

Sample and evaluator provenance are stored through a symlink-safe content-addressed `ContractStore`. Reservation validates immutable `ContractRef` bytes before writing `AttemptReserved`; mutable `plan/sample_manifest.json` and `plan/evaluator_manifest.json` are not canonical inputs. Phase commands use Ledger events plus content-addressed `PhaseRunReceipt v2`; completed receipts are reusable without rerunning side effects, while a started command with no trustworthy receipt becomes an explicit unknown outcome rather than being silently repeated.

Planner pools, scorecards, patch diagnostics, producer proxy-policy reports, mutable result summaries, and human-readable projections remain diagnostic artifacts only. Runtime stages do not infer identity, observations, budgets, authorization, or routing from those files. Projects without DirectionSpec v3, VariantSpec v4, TrialSpec v5, and Event v6 authority must restart from S1; no dual-read or migration exists. See `docs/authoritative_s1_s3_contracts.md` and `docs/regression_test_migration.md`.

Attempt lifecycle operations are generation-scoped. Implementation repair invalidates prior uncommitted phase authority and requires proxy re-execution. Resource resume creates a new generation while preserving the same Attempt and reservation; a full-phase resume retains the already committed proxy outcome and returns to `PROXY_COMPLETED` with full pending. Disposition and finalization events contain verified facts only; `FailureEvidence v4` and `ResumeEvidence v4` are reread from immutable bytes by public APIs, reducer/rebuild, and audit before state, budget, or route can change.
