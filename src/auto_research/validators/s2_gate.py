...
class S2GateValidator(StageGateValidator):
    ...
    def validate(self):
        ...
        trial_errors = contract_errors(trial_spec, "trial_spec_v4.schema.json")
        try:
            validate_trial_spec(trial_spec)
        except ValueError as exc:
            trial_errors.append(str(exc))
        if trial_errors:
            self.retry_check("trial_spec", "TrialSpec v4 validation failed", artifact="plan/trial_spec.json", details={"errors": trial_errors[:20]})
        else:
