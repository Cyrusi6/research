...
- S1c 可以在同一 session 中返回 `status=needs_more_evidence` 和 `followup_evidence_request_plan`；系统会再次调用 deterministic retriever，合并新旧 evidence bundle，再用 `codex resume` 把补充卡片交回 S1c 选方向。
